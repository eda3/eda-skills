...
`{OUT}` `{SKILL_DIR}` `{REPO_ROOT}` `{NN}` `{START}` `{END}` `{THEME}` は、
オーケストレーターが**実際の絶対パス・数値に置き換えてから**渡す（サブエージェントは環境変数を引き継がない）。
各サブエージェントには「割り当てられた成果物ファイルだけを書き、他工程のファイルに触れない」前提で依頼する。
...
<constraints>
- 出力は {OUT}/read/{NN}.jsonl のみ。リポジトリのファイルは読むだけで変更しない。
...
- blocks.tsv: block(01〜) <TAB> start <TAB> end <TAB> theme。ヘッダ行なし。
  ...
  構成順は「読者が理解しやすい順」（全体像→入口→中核→周辺→運用→総まとめ）。ファイルパス順ではない。
...
<constraints>
- 出力は {OUT}/parts/{NN}.html のみ。他ブロックのファイルには触れない。
- 内容の根拠は読了記録に限る。記録にないことは書かないか「未確認」と明言する（nanj-style.md 規約5）。
...
```
<task>
読者代表のレビュー係。{OUT}/<スレ名>.html を通読し、機械検証では拾えない品質を確認して
