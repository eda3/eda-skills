...
3. `data-n` は担当範囲 start〜end の **連番**。start から end まで1ずつ増やし、各番号を1回だけ使う（count_res.sh が gap・dup・範囲を照合する）。
...
6. 図解はすべて下の CSS 部品で描く（画像なしで自己完結させるため。`<script>` `<link>` `<img>` `<iframe>` `<embed>` `<object>` `<video>` `<audio>` は check_html.sh が 0 個を要求する）。
7. コード・設定・ログを引用するときは `<` → `&lt;`、`>` → `&gt;`、`&` → `&amp;` に変換する。
   安価の `&gt;&gt;` 以外で生の `<` が本文に混ざると、タグ扱いされて表示が壊れる。
8. ファイルは常にリポジトリルートからの相対パス
   （inventory.tsv の表記そのまま）で呼ぶ。これが check_html.sh のファイル網羅チェックの照合キーになる。

## インフォグラフィック部品カタログ（class="ig ..." で始める。配分はプロンプトの数値に従う。count_res.sh が判定）
...
### コード引用 `.ig-code`（pre 要素。fn 行にファイルパス。ファイル網羅チェックの照合キーになるため）
```html
...
```
数値は読了記録・inventory.tsv の実測値だけを書く。
