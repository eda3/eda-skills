...
- バッチごとに読了サブエージェントを起動する。同時に最大20体（Claude Code の既定値）。終わったら次のバッチ。
...
- blocks.tsv の1ブロックにつき執筆サブエージェント1体（プロンプト3。同時実行の上限は工程2と同じ）。
  {NN}{START}{END}{THEME} は blocks.tsv の行から展開する。
- 執筆エージェントは自分で `count_res.sh` を最大3回回して RESULT OK まで直す契約。
  NG 報告が返ってきたブロックだけ、新しい執筆エージェントを起動し、元の parts/NN.html と NG 理由を渡して書き直させる（最大2回。直らなければ NG 理由を添えて中断報告）。
